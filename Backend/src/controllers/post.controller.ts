...
export const publishPost = async (
  ...
  try {
    ...
    const postRes = await client.query("SELECT * FROM posts WHERE id = $1", [
      id,
    ]);
    ...
    if (
      ...
      (!seoMeta.description && !seoMeta.excerpt)
    ) {
      await client.query("ROLLBACK");
      return res.status(400).json({
        error:
          "Validation failed: Title, Slug, Banner Image, and Meta Description/Excerpt are required to publish.",
      });
    }
    ...
    `;
    const updatedPost = await client.query(updateQuery, [
      status,
      publishDate,
      id,
    ]);
    ...
    if (status === "published") {
      try {
        ...
        await axios.get(
          `${frontendUrl}/api/revalidate?secret=${process.env.REVALIDATION_SECRET}&slug=${post.slug}`,
        );
        console.log(
          `[LOG - DO]: Revalidation triggered for /blog/${post.slug}`,
        );
      } catch (revalError) {
    ...
    res
      .status(200)
      .json({ message: `Post ${status}`, post: updatedPost.rows[0] });
  } catch (error) {
    await client.query("ROLLBACK");
...
export const getPostsByAuthor = async (req: Request, res: Response) => {
  try {
    ...
    const result = await queryDb(query, [slug, limit, (page - 1) * limit]);
...
export const incrementView = async (req: Request, res: Response) => {
  try {
    ...
    // IP aur User Agent ko combine karke hash banayenge taaki spoofing na ho
    const viewerHash = crypto
      .createHash("sha256")
      .update(`${ip}-${userAgent}`)
      .digest("hex");

    // Insert attempt (agar same din mein same hash exist karta hai to ignore karega)
    ...
    const viewResult = await queryDb(insertViewQuery, [id]);
...
export const getPopularPosts = async (req: Request, res: Response) => {
  try {
    const query = `
            SELECT p.id, p.title, p.slug, p.cover_image_url, p.views_count, p.published_at, u.name as author_name
            FROM posts p
            JOIN users u ON p.author_id = u.id
